...
import 'package:flowerops/screen/delivery_management_screen.dart';
...
class FloristManagementScreen extends StatefulWidget {
  const FloristManagementScreen({Key? key}) : super(key: key);

  @override
  _FloristManagementScreenState createState() =>
      _FloristManagementScreenState();
}

class _FloristManagementScreenState extends State<FloristManagementScreen>
    with SingleTickerProviderStateMixin {
  final EmployeeService _employeeService = EmployeeService();
  List<Employee> _pendingFlorists = [];
  List<Employee> _currentFlorists = [];
  ...
  Future<void> _loadData() async {
    ...
    try {
      final inactiveFlorists = await _employeeService.getInactiveFlorists();
      final activeFlorists = await _employeeService.getActiveFlorists();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Florist Management'),
        actions: [
          Padding(
  ...
  Widget _buildPendingFloristsList() {
    if (_pendingFlorists.isEmpty) {
      return const Center(child: Text('No pending florist applications'));
    }
  ...
  Future<void> _showRejectDialog(Employee employee) async {
  ...
  Future<void> _rejectEmployee(Employee employee, String reason) async {
    try {
      final success =
